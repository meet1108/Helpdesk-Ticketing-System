		...
		<div class="login-container">
			...
			<aui:form action="<%= loginURL %>" autocomplete='<%= PropsValues.COMPANY_SECURITY_LOGIN_FORM_AUTOCOMPLETE ? "on" : "off" %>' cssClass="sign-in-form" method="post" name="<%= formName %>" onSubmit="event.preventDefault();" validateOnBlur="<%= false %>">
				...
				<aui:fieldset>
					...
					%>
					<aui:input id="email" cssClass="clearable" label="<%= loginLabel %>" name="login" showRequiredLabel="<%= false %>" type="text" value="<%= login %>">
...
<%-- 					<aui:input name="password" showRequiredLabel="<%= false %>" type="password" value="<%= password %>"/> --%>
					...
					<liferay-ui:error key="error" message="otp-error" />
					<liferay-ui:success key="success" message="otp-success"/>
					
					<aui:input label="OTP" placeholder="Enter OTP" name="opt"  type="text" >
						<aui:validator name="required" />
				...
				<aui:button-row>
					...
					<aui:button id="GetData" value="Generate Otp" onclick="generateOTP()"/>
				</aui:button-row>
			</aui:form>
		...
		AUI().use('aui-io-request',function(A){
	        A.io.request('<portlet:resourceURL id="otpGenerate" />', {
	            method: 'POST',
	            data: {
